...
def is_in_range(coords, range_coords):
    surname_first_letter = coords[0][0]  # Get the first letter of the surname
    awards = coords[1]

    min_surname_letter = range_coords[0][0]
    max_surname_letter = range_coords[0][1]

    min_awards = range_coords[1][0]
    max_awards = 100

    # Check if the scientists meet the criteria
    if (
            (ord(surname_first_letter) < ord(range_coords[0][0]) or ord(surname_first_letter) > ord(range_coords[0][1]))
            or (
            int(awards) < int(range_coords[1][0]) or int(awards) > int(range_coords[1][1])
    )
            and int(awards) != max_awards
    ):
        return False
    return True
...
def range_search(root, range_coords, dimension=0):
    ...
    if dimension + 1 == DIMENSIONS:
        if int(root.coords[dimension]) < int(range_coords[dimension][0]):
            return range_search(root.right, range_coords, dimension)
        elif int(root.coords[dimension]) > int(range_coords[dimension][1]):
